...
app.get('/api/docs', (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      { path: "/register", method: "POST", payload: "{ username, bio }", description: "Synchronize a new identity with the finalcut.ai network and obtain an API key." },
...
app.post(['/api/auth/register', '/register'], async (req, res) => {
  try {
    ...
    if (userCheck.rows.length > 0) return res.status(400).json({ error: 'Identity collision detected.' });
